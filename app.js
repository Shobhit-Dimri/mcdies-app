...
app.post("/webhook/", (req, res) => {
	...
	switch (actionName) {
		case "require_permission":
			{
				console.log("***** Inside Require Permission *****");
		...
		case "check_sign_in":
			{
				console.log("***** Inside Check Sign In *****");
				if (isDefined(actionName)) {
					//console.log(JSON.stringify(req.body));
					...
					var surfaces =
						req.body.originalRequest.data.availableSurfaces[0].capabilities;
		...
		case "check_permission":
			{
				...
				if (isDefined(actionName)) {
					...
					if (req.body.originalRequest.data.inputs[0].arguments[0].boolValue) {
						if (
							req.body.originalRequest.data.user.permissions[0] ==
							...
							uLat =
								req.body.originalRequest.data.device.location.coordinates
									.latitude; // = 41.8834;
							uLng =
								req.body.originalRequest.data.device.location.coordinates
									.longitude; // = -87.6537;
						...
						qsr.nearestStoreService(uLat, uLng, (error, storeResult) => {
							if (error) {
								...
								qsr.calculateDistanceService(
									uLat,
									uLng,
									storeResult.sLat,
									storeResult.sLng,
									(error, durationResult) => {
										if (error) {
											console.log(error);
										} else {
											console.log(durationResult.duration);
											qsr.createCartService(
												access_token,
												email,
												(error, cartResult) => {
													if (error) {
														...
														qsr.settingDeliveryModeService(
															access_token,
															cartId,
															email,
															(error, result) => {
											...
											text = `Thank you for your permission ! I can place an order for you at the nearest ${
												storeResult.name
												} at ${storeResult.address}, which is a ${
												durationResult.duration
												} walk from your place. What would you like to order ?`;
											messageData = {
												speech: text,
												displayText: text
											};
						...
						text =
							"I am sorry ! I cannot process your order without your permission";
						messageData = {
							speech: text,
							displayText: text
						};
		...
		case "productsOrderMac":
			{
				...
				if (isDefined(actionName)) {
					console.log(
						"Access Token  generated-  " + access_token + "for- " + productName
					);
					console.log(cartId);
					...
					qsr.getRecommendedProductService(productName, (error, result) => {
						if (error) {
							...
							qsr.getProductCodeByNameService(
								productName,
								(error, prodResult) => {
									if (error) {
										console.log(error);
										...
										text =
											"I am sorry ! This item does not exist! What would you like to order?";
										messageData = {
											speech: text,
											displayText: text
										};
										...
										qsr.addProductsToCart(
											access_token,
											cartId,
											email,
											prodResult.productCode,
											storeName,
											(error, nextProductResult) => {
												if (error) {
													console.log(error);
												} else {
				...
				if (isDefined(actionName)) {
					qsr.fetchCartService(access_token, cartId, email, (error, result) => {
						if (error) {
							...
							qsr.gettingSavedCardDetailsService(
								access_token,
								email,
								(error, cardResult) => {
									if (error) {
										...
										qsr.addCardPaymentService(
											...
											(error, paymentResult) => {
												if (error) {
													...
													console.log(
														"Payment details added with storeId: ",
														storeId
													);
												}
											}
										...
										text = `The total will be ${
											result.totalPrice
											} $. Would you like to use your default card on file ending with ${defCardNumber.substr(
												12,
												4
											)}?`;
										messageData = {
					...
					qsr.placeOrderService(
						...
						(error, orderResult) => {
							if (error) {
								...
								res.send(messageData);
								qsr.gettingOrbIdFromOrderService(
									storeId,
									orderResult.code,
									(error, orbIdResult) => {
										if (error) {
